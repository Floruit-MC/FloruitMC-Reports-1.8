...
public class ReportCommand extends BaseCommand {
    ...
    public void onSendCommand(Player player, String[] args) {
        ...
        if (hasReportedRecently(player)) {
            player.sendMessage(new String[]{"", "§cVocê já reportou um jogador recentemente.", "§cAguarde um pouco antes de reportar novamente.", ""});
            return;
        }
    ...
    private void sendReportOptionsMessage(Player player, List<String> reportOptions, Player target) {
        ...
        player.sendMessage("");
        player.sendMessage(new String[]{"§cAlgumas infrações mais graves como §eRacismo/Discriminação", "§eComércio Externo §ce entre outras devem ser delatadas", "§ccom provas que seguem o padrão do servidor."});
        player.sendMessage("");
